...
//TT MODULOS
import { CONTACTOS, CONFIG_ENV, APPSHEETCONFIG } from './bot.mjs'
...
export function ComprobrarListaNegra(ctx) {
  ...
  if (dato) {
    ...
    postTable(APPSHEETCONFIG, CONFIG_ENV.CONTACTOS, _new)
  }
  return false
}
...
export async function AgregarListaNegra(ctx, hablar = false) {
  ...
  if (dato) {
    ...
    patchTable(APPSHEETCONFIG, CONFIG_ENV.CONTACTOS, dato)
    return true
  } else {
    console.info(`contacto nuevo: ${ctx.name} con numero: ${ctx.from}`)
    const _new = {
