...
//TT PAUSA CONDICIONAL
/**
 * Pausa la ejecución de código de manera condicional hasta que se cumpla una condición
 * o hasta que pase un tiempo máximo.
 *
 * @param {number} sg - Tiempo máximo en segundos antes de forzar la finalización de la pausa.
 * @param {function} check - Función que se ejecuta periódicamente para verificar una condición.
 * @param {*} valor - Valor esperado para que la condición se considere cumplida.
 * @param {number} [comp=1000] - Intervalo en milisegundos para verificar la condición.
 * @returns {Promise<void>} - Promesa que se resuelve cuando la condición es verdadera o se agota el tiempo.
 */
export function PausaCond(sg, check, valor, comp = 1000) {
...
//SS CREAR LISTA DE INTERVALOS
/**
 * Genera una lista de todas las horas del día con un intervalo específico.
 *
 * @param {number} intervaloMinutos - El intervalo en minutos que se sumará a cada hora.
 * @returns {string[]} - Un arreglo con las horas del día en formato "HH:MM", con el intervalo aplicado.
 *
 * @example
 * // Genera las horas del día con un intervalo de 20 minutos
 * const horasDelDia = generarHorasDelDiaConIntervalo(20);
 * console.log(horasDelDia);
 * // Resultado: ["00:00", "00:20", "00:40", "01:00", ..., "23:40"]
 */
export function GenerarHorasConIntervalo(intervaloMinutos) {
  const resultado = []
  const horaActual = new Date()
  horaActual.setHours(0, 0, 0, 0) // Inicializar a las 00:00

  while (horaActual.getDate() === new Date().getDate()) {
    // Agregar la hora actual al resultado (solo horas y minutos)
    resultado.push(horaActual.toTimeString().slice(0, 5))
